...
for (gene in diff_gene_names) {
  ...
  sim_matrix[data$obese == "Yes", idx] <- rnorm(sum(data$obese == "Yes"), mean = 2, sd = 1)
  sim_matrix[data$obese == "No", idx]  <- rnorm(sum(data$obese == "No"), mean = 0, sd = 1)
}

for (gene in obesity_genes) {
  idx <- which(colnames(sim_matrix) == gene)
  sim_matrix[data$obese == "Yes", idx] <- rnorm(sum(data$obese == "Yes"), mean = 4, sd = 0.5)
  sim_matrix[data$obese == "No", idx]  <- rnorm(sum(data$obese == "No"), mean = 0, sd = 1)
}
...
input_diabetes <- read_csv("data-obesity.csv")
